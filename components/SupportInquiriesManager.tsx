...
import { updateSupportInquiryStatus } from '@/lib/firestore'
import {
  HelpCircle,
  Mail,
  User,
...
export function SupportInquiriesManager({ inquiries, onUpdate }: SupportInquiriesManagerProps) {
  ...
  const products = Array.from(new Set(inquiries.map((i) => i.product))).sort()
                ...
                filtered.map((inq) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-slate-300 text-sm">{inq.product}</td>
