...
const ParticleTextEffect: React.FC<ParticleTextEffectProps> = ({
  ...
  const interactionRadiusRef = useRef<number>(80);
    ...
    move(interactionRadius: number, hasPointer: boolean) {
      ...
      if (hasPointer && pointerRef.current.x !== undefined && pointerRef.current.y !== undefined) {
        ...
        if (dist < interactionRadius && dist > 0) {
          const force = Math.min(this.f, (interactionRadius - dist) / dist * (isMobile ? 1.5 : 2));
      ...
      if (od > 1) {
        const restore = Math.min(od * 0.08, 2.5);
  ...
  const handlePointerMove = (e: React.PointerEvent<HTMLCanvasElement>) => {
    ...
    // Throttle pointer events on mobile
