...
function SplashCursor({
  ...
  useEffect(() => {
    // Check for mobile and reduced motion preferences - be less restrictive
    const checkMobile = () => setIsMobile(window.innerWidth <= 480); // Only disable on very small screens
    ...
    // Use reduced settings on mobile instead of completely disabling
    const isMobileDevice = window.innerWidth <= 768;
    const hasReducedMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    
    if (hasReducedMotion) return; // Only disable for reduced motion preference
    ...
    let config = {
      SIM_RESOLUTION,
      DYE_RESOLUTION: isMobileDevice ? Math.min(DYE_RESOLUTION, 256) : DYE_RESOLUTION,
      CAPTURE_RESOLUTION,
      DENSITY_DISSIPATION,
      ...
      CURL,
      SPLAT_RADIUS,
      SPLAT_FORCE: isMobileDevice ? SPLAT_FORCE * 0.5 : SPLAT_FORCE,
      SHADING,
      COLOR_UPDATE_SPEED: isMobileDevice ? COLOR_UPDATE_SPEED * 0.5 : COLOR_UPDATE_SPEED,
      PAUSED: false,
      BACK_COLOR,
      TRANSPARENT,
    };
    ...
    const handleMouseMove = (e: MouseEvent) => {
      let pointer = pointers[0];
      // Throttle on mobile for performance
      if (isMobileDevice && Math.random() > 0.7) return;
  ...
  // Only don't render if user prefers reduced motion
  if (reducedMotion) {
    return null;
  }
